...
%% inverse mapping
%forgat local loss
filename='20170826_190417/g_2dgresbnd100000x50img50_face_20170826_211846.txt';
filename='20170826_171232/g_2dgresbnd1x1img1_face_20170827_210045.txt';
% half of bnd loss is wrong
filename='20170827_215921/g_2dgresbnd10x10img10_face_20170827_235844.txt';
%without local info: the highest points are not well ordered. Is it because with bnd punishment, maxpooling learns to pay much attention to points on the bottom but not the highest 
filename='20170828_003304/g_2dgresbnd10x10img10local10_face_20170828_063517.txt';
%filename='20170828_004853/g_2dgresbnd1000x10img10local100_face_20170828_064632.txt';

%with local info
%good, first is better than the second
filename='20170828_002303_/g_2dgresbnd10x10img10local10_face_20170828_092327.txt';
%filename='20170828_005152_/g_2dgresbnd100x10img10local100_face_20170828_113833.txt';
%to make sure: bn before regression? relu between first conv and first
%gconv
dirname='../pointnet.pytorch/inversemap/results/';
...
a=textscan(fidout,'%f', 'TreatAsEmpty','*','EmptyValue',10);
out=[a{1,1}];
...
plotr=2;plotc=3;
pos=-1;
for k =5:1:1000 %(size(in,1)/num_points)-37:(-3):1 %78:1:1000 % 
...
% 5three,10 plane4two,24chess,172T,125C
    ...
    b_ori=out((k-1)*num_grid*3+1:k*num_grid*3,:);
...
subplot(plotr,plotc,pos*plotc+inc);inc=inc+1;
    ...
    b=b_ori;
    subplot(plotr,plotc,pos*plotc+inc);inc=inc+1; 
    hold on, trisurf(tri_,b(1:num_grid),b(num_grid+1:2*num_grid),b(num_grid*2+1:3*num_grid),'FaceColor','interp','FaceAlpha',1), hold off %,'FaceColor','interp'
    axis equal;axis tight;
    title(['3D Mesh' num2str(k) ]);
    [E] = exterior_edges(tri_);
    s = [b(E(:,1),:), b(E(:,1)+num_grid,:), b(E(:,1)+2*num_grid,:)]; e = [b(E(:,2),:), b(E(:,2)+num_grid,:), b(E(:,2)+2*num_grid,:)];
    line([s(:,1)';e(:,1)'], [s(:,2)';e(:,2)'], [s(:,3)';e(:,3)'], 'Color', 'r', 'LineWidth', 3);
    hold off;
    axis equal; axis tight; %axis off; 

    subplot(plotr,plotc,pos*plotc+inc);inc=inc+1;
    scatter3(b(1:num_grid),b(num_grid+1:2*num_grid),b(num_grid*2+1:3*num_grid));
